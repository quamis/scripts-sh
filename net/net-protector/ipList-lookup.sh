...
while IFS= read -r IP; do
	case "$SCANNER" in
		...
		abusedb)
			CACHEKEY="$CACHE_K1/${IP}-abusedb@$CACHE_K1.cache"
			...
			readarray -t X < <(echo "$JSON" | jq ".data.totalReports, .data.abuseConfidenceScore, .data.usageType, .data.isp, .data.countryCode")
			echo "${X[4]}, isp:${X[3]}, reports:${X[0]}, confidence:${X[1]}, $IP, usage:${X[2]}," >> "$TMPLIST"
		;;

		ipstack)
			CACHEKEY="$CACHE_K1/${IP}-ipstack@$CACHE_K1.cache"
			if [ -f "${CACHE_DIR}/${CACHEKEY}" ]; then
				JSON=`cat "${CACHE_DIR}/${CACHEKEY}"`;
				echo -n "c";
			else
		...
		ipdata)
			CACHEKEY1="$CACHE_K1/${IP}-ipdata@$CACHE_K1-threat.cache"
			...
			CACHEKEY2="$CACHE_K1/${IP}-ipdata@$CACHE_K1-asn.cache"
		...
		#fraudguard)
		#	CACHEKEY="$CACHE_K1/${IP}-fraudguard@$CACHE_K1.cache"
		#	if [ -f "${CACHE_DIR}/${CACHEKEY}" ]; then
		#		JSON=`cat "${CACHE_DIR}/${CACHEKEY}"`;
		#		echo -n "c";
		#	else
